...
```sh
uv tool install rovr --compile-bytecode
```

<Aside type="note">
I highly recommend installling with `--compile-bytecode`, as startup times will be slower (albeit by not that much) if not cached.
</Aside>
