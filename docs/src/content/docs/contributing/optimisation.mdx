...
<br/>hence, `rovr` makes use of a couple of optimisations to make it perform reasonably well
...
<Aside type="note">
this has been slated to be removed, mainly due to issues with threads in general.<br/>
textual isn't able to handle threads as well, hence for heavier previews, it needs to cut into the main process, which leads to too much lag.
</Aside>
...
- when doing heavy tasks, like calculating folder size, or deleting folders, the appropriate widgets' labels are updated too frequently.
- by using a 'debouncer', more time is spent on actually doing the process, than to update the progress of the process

This is certainly not a one-of-a-kind optimisation, but it is a nice thing to have.
