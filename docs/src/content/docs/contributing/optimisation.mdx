...
```mermaid
flowchart TD
    a(run thread) --> b{check for active thread}
...
flowchart TD
    ...
    b --> c{do heavy process}
    c --> |process completed|h[force label update]
    h --> s(stop)
    c --> d{if last_updated + previous_updated_time < current_time}
    d --> |true|e[update label]
    e --> g[time_diff += 0.01 if time_diff < 1.00]
    g --> c
