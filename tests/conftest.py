...
@pytest.fixture
def sample_archive_structure(temp_dir: Path) -> dict:
    """Create various archive types for testing.
    
    Returns dict with archive paths:
...
@pytest.fixture
def app_with_files(sample_file_structure: Path, monkeypatch):
...
def disable_file_watcher(monkeypatch):
