...
class ProcessContainer(VerticalScroll):
    ...
    def delete_files(
        ...
        for file in files:
            ...
            files_to_add, folders_to_add = self.app.call_from_thread(
                lambda: path_utils.get_recursive_files(file, with_folders=True)
            )
    ...
    def paste_items(self, copied: list[str], cutted: list[str], dest: str = "") -> None:
        ...
        for file in copied:
            files_to_copy.extend(
                self.app.call_from_thread(lambda: path_utils.get_recursive_files(file))
            )
        for file in cutted:
            if path.isdir(file):
                cut_files__folders.append(path_utils.normalise(file))
            files, folders = self.app.call_from_thread(
                lambda: path_utils.get_recursive_files(file, with_folders=True)
            )
