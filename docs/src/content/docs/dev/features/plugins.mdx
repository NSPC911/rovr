...
```toml title="config.toml"
[plugins.resvg]
# refer to https://resvg-py.readthedocs.io/en/latest/api.html
dpi = 0 # svg default
...
uses the [`imagemagick`](https://imagemagick.org) command to provide image preview support for unsupported image formats (like jxl, heic, etc.)
