...
def sample_tar_gz(temp_dir: Path) -> Path:
    ...
    Returns:
        Path to the sample tar.gz file.
    """
...
class TestArchiveOpen:
    ...
    def test_open_valid_zip(self, sample_zip: Path) -> None:
    ...
    def test_open_valid_tar_gz(self, sample_tar_gz: Path) -> None:
    ...
    def test_open_nonexistent_file_raises(self, temp_dir: Path) -> None:
        """Non-existent file raises FileNotFoundError."""
        from rovr.classes.archive import Archive

        with pytest.raises(FileNotFoundError), Archive(str(temp_dir / "nonexistent.zip")):
            pass

    def test_open_empty_zip(self, empty_zip: Path) -> None:
...
class TestArchiveList:
    ...
    def test_list_zip_contents(self, sample_zip: Path) -> None:
    ...
    def test_list_tar_contents(self, sample_tar_gz: Path) -> None:
...
class TestArchiveRead:
    ...
    def test_open_zip_member(self, sample_zip: Path) -> None:
        """Can open and read content of ZIP member."""
        from rovr.classes.archive import Archive

        with Archive(str(sample_zip)) as archive, archive.open("file1.txt") as f:
            assert f is not None
            content = f.read()
            assert content == b"Hello, World!"

    def test_open_nonexistent_member_raises(self, sample_zip: Path) -> None:
        """Opening non-existent member raises KeyError."""
        from rovr.classes.archive import Archive

        with Archive(str(sample_zip)) as archive, pytest.raises(KeyError):
...
class TestArchiveExtract:
    ...
    def test_extract_all_members(self, sample_zip: Path, temp_dir: Path) -> None:
    ...
    def test_extract_single_member(self, sample_zip: Path, temp_dir: Path) -> None:
...
class TestArchiveContextManager:
    ...
    def test_context_manager_closes(self, sample_zip: Path) -> None:
...
class TestArchiveTypeDetection:
    ...
    def test_detects_zip_by_extension(self, sample_zip: Path) -> None:
    ...
    def test_detects_tar_gz_by_extension(self, sample_tar_gz: Path) -> None:
