...
- src/rovr
  ...
  - options.py classes for option/selection elements
  - py.typed
  - search_container.py search bar in pinned folders and file list
  - style.tcss default style
  - themes.py theme loader
  - utils.py utilities that cannot be split
  - validators.py validations for input elements
</FileTree>
...
- src/rovr/action_buttons
  ...
  - paste_button.py paste from clipboard into cwd
...
not much extras, might move some uncategorised stuff here
...
I'm currently waiting for textual's next release for a thicker progress bar, so that gradient bars can be added
