...
[detail columns](/rovr/dev/features/detail-columns) get their own component classes. these do not have `--hovered` or `--highlighted` sub classes; only the foreground color and text style apply, on top of the row's background.
