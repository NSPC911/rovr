...
`$primary` is the only required variable. every color you don't define is generated from the ones you do — including shades like `$primary-lighten-3`, which regenerate automatically when you change `$primary`.
...
any other declaration becomes a custom variable, usable by [textual's additional variables](https://textual.textualize.io/guide/design/#additional-variables) and rovr's own css.
