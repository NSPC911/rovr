...
I also recommend installing with `--python 3.14t` (the free threaded build) because proper concurrency is supported aside from spawning new `Processes` (which tanks system resources)
</Aside>
...
```sh
sudo dnf copr enable NSPC911/rovr
sudo dnf install rovr
```
