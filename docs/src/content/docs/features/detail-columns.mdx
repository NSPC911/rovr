...
each column can be styled with component classes on `FileList`; the git column additionally styles its staged and unstaged characters separately. see [theming](/rovr/configuration/theming/#styletcss) for the full list. the header row is a plain widget with the id `#file_list_details_header`.
