...
def get_schema_validator() -> tuple[dict, Callable[[dict], None]]:
    ...
    if path.isfile(schema_py):
        try:
            spec = util.spec_from_file_location("schema", schema_py)
            if spec is not None:
                module = util.module_from_spec(spec)
                spec.loader.exec_module(module)
                if module.mtime == mtime:
                    return module.schema_dict, module.validate
        except Exception:
            pass
    schema_dict = json.loads(schema_file.read_text("utf-8"))
    try:
        if path.exists(schema_py):
            if path.isdir(schema_py):
                import shutil

                shutil.rmtree(schema_py)
            else:
                os.remove(schema_py)
        os.makedirs(RovrVars.ROVRTEMP, exist_ok=True)
        with open(schema_py, "w", encoding="utf-8") as f:
            print("Pre-compiling config schema for faster validation...", end="")
            f.write(f"""# This file is auto-generated by rovr for faster schema validation. Do not edit.
# DO NOT CHANGE ANYTHING WITH THIS FILE.

mtime = {mtime}

schema_dict = {schema_dict}

{fastjsonschema.compile_to_code(schema_dict)}
""")
