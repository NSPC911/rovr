...
- <kbd>backspace</kbd> or <kbd>u</kbd>: go back to previous location
- <kbd>space</kbd>: go forward in history (after going back)

### pinned folders
pin frequently accessed directories for quick access:
- <kbd>shift+p</kbd>: pin the current folder
- pinned folders appear in the sidebar
- pins are saved and restored, and automatically updated across sessions

### quick jumping
- <kbd>ctrl+l</kbd>: focus path switcher, similarly to `explorer.exe`
- <kbd>f</kbd>: if `plugins.fd` is enabled, search for something in the current directory recursively using `fd`
- <kbd>z</kbd>: if `plugins.zoxide` is enabled, use `zoxide` to go to another directory
...
- create custom themes as `tcss` files in `<config folder>/themes/` — they hot-reload while rovr is running
...
- <kbd>ctrl+f</kbd> or <kbd>/</kbd>: search in current directory
