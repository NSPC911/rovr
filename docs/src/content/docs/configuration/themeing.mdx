...
you can take a look at the reference in [textual](https://textual.textualize.io/guide/design)
...
Vertical(id="root")
  ...
  HorizontalGroup(id="main")
    ...
    VerticalGroup(id="file_list_container")
      SearchInput() -> Input
      FileList(id="file_list", name="File List", classes="file-list") -> SelectionList
        FileListSelectionWidget() -> Selection
    PreviewContainer(id="preview_sidebar") -> Container
      Image(id="image_preview", classes="inner_preview")
      CustomTextArea(id="text_preview") -> TextArea
      Static(id="text_preview", classes="inner_preview")
      FileList(
        id="folder_preview",
